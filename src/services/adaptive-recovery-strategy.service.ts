        ...
        if (strategy === 'CONSERVATIVE_TO_AGGRESSIVE') {
            ...
            if (signalType === 'OVER') {
                ...
                if (isBarrierAdjusted) {
                    ...
                    // Normal OVER signals: step up for recovery (more aggressive)
                    predictionAfterLoss = Math.min(barrier + 1, 9);
                    reasoning = `OVER${barrier}: Start at barrier ${barrier}, escalate to ${predictionAfterLoss} after loss`;
                }
            } else {
                predictionBeforeLoss = barrier; // Conservative: exactly at adjusted barrier
                ...
                if (isBarrierAdjusted) {
                    ...
                    // Normal UNDER signals: step down for recovery (more aggressive)
                    predictionAfterLoss = Math.max(barrier - 1, 0);
                    reasoning = `UNDER${barrier}: Start at barrier ${barrier}, escalate to ${predictionAfterLoss} after loss`;
                }
            }
        } else {
            ...
            if (signalType === 'OVER') {
                if (isBarrierAdjusted) {
                    ...
                    predictionBeforeLoss = Math.min(barrier + 1, 9); // Aggressive: one step higher
                    predictionAfterLoss = barrier; // Conservative: back to barrier
                    reasoning = `OVER${barrier}: Start aggressive at ${predictionBeforeLoss}, fallback to barrier ${barrier}`;
                }
            } else {
                if (isBarrierAdjusted) {
                    ...
                    predictionBeforeLoss = Math.max(barrier - 1, 0); // Aggressive: one step lower
                    predictionAfterLoss = barrier; // Conservative: back to barrier
                    reasoning = `UNDER${barrier}: Start aggressive at ${predictionBeforeLoss}, fallback to barrier ${barrier}`;
                }
            }
        }
